    ...
    for(int i = 0; i < int(player_list.size()); ++i)
      for(int j = 1; j < int(player_list.size()); ++j) {
        ...
        if (dis_y != 0. && frac < 4.f / 3.f) {
          if (450.f / dis_y < kScale_threshold_low) {
            if (scale > kScale_threshold_low)
              scale = kScale_threshold_low;
          }
          else if (450.0f / dis_y > kScale_threshold_high) {
            if (scale > kScale_threshold_high)
              scale = kScale_threshold_high;
          }
          else {
            if (scale > 450.f / dis_y)
              scale = 450.f / dis_y;
            }
        }
        else {
          if (600.f / dis_x < kScale_threshold_low) {
            if (scale > kScale_threshold_low)
              scale = kScale_threshold_low;
          }
          else if (600.f / dis_x > kScale_threshold_high) {
            if (scale > kScale_threshold_high)
              scale = kScale_threshold_high;
          }
          else {
            if (scale > 600.f / dis_x)
              scale = 600.f / dis_x;
          }
        }
